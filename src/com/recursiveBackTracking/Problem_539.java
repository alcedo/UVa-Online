...
/**
 * This is like a brute force depth first search traversal sln... 
 */
public class Problem_539 {
    ...
    public static void main(String[] args) throws IOException {
		...
		while((nodes = input.readInt()) != 0) {
			...
			for(int i=0; i<edges; i++) {
				from[i] = input.readInt();
				to[i] = input.readInt();
			}
			
			
			int max=0;
			for(int i=0; i<edges; i++) //this loop is to try out each node as our Search Starting point
				max = Math.max(max, maxLength(new HashSet<Integer>(), i, from, to, new ArrayList<Integer>(), new ArrayList<Integer>()));
    ...
    public static int maxLength(HashSet<Integer> edgeSet, int nodeId, int[] from, int[] to, 
    		ArrayList<Integer> cFrom, ArrayList<Integer> cTo){
    
    	int maxLength = 0;
    	for(int i=0; i<from.length; i++) {

    		if(edgeSet.contains(i)) //ignore edges that has been visited before 
    			continue;
    		...
    		if(from[i] == nodeId) {
    			...
    			maxLength = Math.max(maxLength, maxLength(edgeSet, to[i], from, to, cFrom, cTo  ) +1 );
    			
    			edgeSet.remove(i);
    			cFrom.remove(cFrom.size()-1);
    			cTo.remove(cTo.size()-1);
    		}
    	}
    	
    	// try the other way too! as long as edges hasn't been traversed before 
    	// we are trying the other way because graph is un-directed.
    	for(int i=0; i<to.length; i++) {
    		...
    		if(to[i] == nodeId) {
    			...
    			maxLength = Math.max(maxLength, maxLength(edgeSet, from[i], from, to, cFrom, cTo  ) +1 );
...
// unComment to see traversal path    	
//    	for(int i=0; i<cFrom.size(); i++){ 
//    		out.print(" " + cFrom.get(i) + " -> " + cTo.get(i) + " ");
//    	}out.println("");
